...
net = patternnet(7);       % The argument of the function = # of hidden neurons
net.divideFcn = '';          % To prevent the network from automatically dividing data into sets at random

tic     % Start Timer
[net,tr] = train(net,P',T');    % Train MLP network
toc     % Stop Timer
...
training_actualOutput(j,1) = net(netData(j,1:numOfInputs)');    % Use network to predict training data
...
training_actualOutput(j,1) = (training_actualOutput(j,1)*std(data))+mean(data);
training_expectedOutput(j,1) = (training_expectedOutput(j,1)*std(data))+mean(data);
...
testing_actualOutput(k,1) = net(netData(k+testingPoint,1:numOfInputs)');    % Use network to predict testing data
