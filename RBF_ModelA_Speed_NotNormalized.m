...
for i = 1:dataSize-numOfInputs
    netData(i,:) = data(i:(i+numOfInputs))';
end
...
training_actualOutput(j,1) = training_actualOutput(j,1);
training_expectedOutput(j,1) = training_expectedOutput(j,1);
...
testing_actualOutput(k,1) = testing_actualOutput(k,1);
testing_expectedOutput(k,1) = testing_expectedOutput(k,1);
