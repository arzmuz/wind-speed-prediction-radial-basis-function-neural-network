...
data = importdata('Madrid_Speed_Direction.xlsx');   % Import data
...
numOfInputs = 8;            % Number of network inputs
testingPoint = dataSize - testingSamples - (numOfInputs/2);     % Point to start prediction from

% Creation of matrix for the network to train and predict
netData = zeros(dataSize(1)-(numOfInputs/2),numOfInputs+1);
for i = 1:(dataSize(1)-(numOfInputs/2))  
    netData(i,1:(numOfInputs/2)) = normalized_speed(i:(i+((numOfInputs/2)-1)))';
    netData(i,((numOfInputs/2)+1):numOfInputs) = normalized_dir(i:(i+((numOfInputs/2)-1)))';
    netData(i,(numOfInputs+1)) = normalized_speed(i+((numOfInputs/2)));
end

% Creation of training matrices
P = netData(1:trainingSamples-(numOfInputs/2),1:numOfInputs);
T = netData(1:trainingSamples-(numOfInputs/2),numOfInputs+1);

clearvars data normalized_speed normalized_dir;  % To free memory of variables no longer needed

net = newrb(P',T',0,60,60);

% Declaration of matrices to calculate training forecast errors
training_actualOutput = zeros(trainingSamples-(numOfInputs/2),1);
training_expectedOutput = zeros(trainingSamples-(numOfInputs/2),1);
trainingError = zeros(trainingSamples-(numOfInputs/2),1);

for j = 1:trainingSamples-(numOfInputs/2)
...
plot(training_expectedOutput(1:trainingSamples - (numOfInputs/2)),'r-')
hold on;
plot(training_actualOutput(1:trainingSamples - (numOfInputs/2)),'b')
...
testing_actualOutput = zeros(testingSamples-(numOfInputs/2),1);
testing_expectedOutput = zeros(testingSamples-(numOfInputs/2),1);
testingError = zeros(testingSamples-(numOfInputs/2),1);

for k = 1:testingSamples-(numOfInputs/2) 
...
plot(testing_expectedOutput(1:testingSamples - (numOfInputs/2)),'r-')
hold on;
plot(testing_actualOutput(1:testingSamples - (numOfInputs/2)),'b')
